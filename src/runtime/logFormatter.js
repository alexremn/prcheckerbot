...
// drop probe noise) before forwarding to `write`. A record may be split across
...
// A factory, not a singleton: the line buffer is per-destination state.
function createLogDestination(write = (line) => process.stdout.write(line)) {
  let pendingLine = "";
  // Writable hands _write a Buffer, and a chunk boundary can fall inside a
  // multi-byte character — chunk.toString() would then decode U+FFFD into both
  // halves, so a non-ASCII PR title survived neither half. StringDecoder holds
  // the incomplete bytes back until the rest of the character arrives.
  ...
  return new Writable({
    ...
    final(callback) {
      const tail = pendingLine + decoder.end();
